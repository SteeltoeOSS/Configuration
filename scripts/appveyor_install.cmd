...
IF "%APPVEYOR_REPO_BRANCH%"=="vnext" COPY config\nuget-vnext.config .\nuget.config
...
IF "%APPVEYOR_REPO_BRANCH%"=="vnext" COPY config\versions.props .\versions.props
IF NOT "%APPVEYOR_REPO_TAG_NAME%"=="" COPY config\versions.props .\versions.props
IF "%APPVEYOR_REPO_BRANCH%"=="dev" SET BUILD_TYPE=Debug
IF "%APPVEYOR_REPO_BRANCH%"=="vnext" SET BUILD_TYPE=Debug
